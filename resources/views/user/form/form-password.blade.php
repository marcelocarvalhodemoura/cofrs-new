<form class="needs-validation text-left password" novalidate id="formSavePassword">
    @csrf
    <input type="hidden" name="editUserID" id="editUserID" value=""/>
    <div class="form-row mb-4">
        <div class=" form-group col-md-6 ">
            <label for="editPassword">Senha</label>
            <input type="password" class="form-control" id="editPassword" name="editPassword" placeholder="inform a senha" required />
            <div class="elemValidador">Força da senha: <meter value="0" class="mtSenha" max="100" low="30" high="60" optimum="100" title="Força da senha"></meter></div>
        </div>
        <div class="col-md-6 mb-4">
            <label for="editPassword2">Conf. de Senha</label>
            <input type="password" class="form-control" id="editPassword2" name="editPassword2" data-match="#password" data-match-error="Atenção! As senhas não estão iguais." placeholder="informe conf. de senha" required />
        </div>
    </div>
    <div class="modal-footer">
        <button class="btn" data-dismiss="modal"><i class="flaticon-cancel-12"></i>Limpar</button>
        <button class="btn btn-primary" type="submit">Salvar</button>
    </div>
</form>
